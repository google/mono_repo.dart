  ...
  ($checkedConvert) {
    ...
    final val = GitHubConfig(
      ...
      $checkedConvert(
        'on_completion',
        (v) =>
            (v as List<dynamic>?)?.map((e) => Job.fromJson(e as Map)).toList(),
      ),
      ...
      $checkedConvert(
        'workflows',
        (v) => (v as Map?)?.map(
          (k, e) => MapEntry(k as String, GitHubWorkflow.fromJson(e as Map)),
        ),
      ),
      $checkedConvert(
        'dependabot',
...
GitHubWorkflow _$GitHubWorkflowFromJson(Map json) =>
    $checkedCreate('GitHubWorkflow', json, ($checkedConvert) {
      $checkKeys(
        json,
