...
void main() {
  test('phase 5 features: ignore, pre_steps, post_steps, transitive paths', () async {
    await d.dir('pkg_a', [
...
'''),
    ...
    final ignoredWorkflow = File(p.join(d.sandbox, githubWorkflowFilePath('pkg_ignored')));
    expect(ignoredWorkflow.existsSync(), isFalse);

    // Verify pkg_hooks has pre_steps and post_steps
    await d.file(githubWorkflowFilePath('pkg_hooks'), contains('pre_step')).validate();
    await d.file(githubWorkflowFilePath('pkg_hooks'), contains('post_step')).validate();

    // Verify pkg_a has transitive paths (pkg_b)
    await d.file(githubWorkflowFilePath('pkg_a'), contains('pkg_b/**')).validate();
  });
}
